...
profileRouter.get('/', requireAuth, async (req, res, next) => { try { return res.json({ profile: await prisma.user.findUnique({ where: { id: req.user!.id }, select: { id: true, name: true, email: true, role: true, subscriptionTier: true, firstName: true, lastName: true, phone: true, avatarUrl: true, address: true, state: true, country: true, postalCode: true } }) }); } catch (error) { return next(error); } });
