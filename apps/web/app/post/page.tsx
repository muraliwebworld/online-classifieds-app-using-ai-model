...
export default function PostPage() {
  ...
  useEffect(() => { void Promise.all([fetch(`${API_URL}/api/catalog/categories`).then((r) => r.json()), fetch(`${API_URL}/api/catalog/locations`).then((r) => r.json())]).then(([c, l]) => { setCategories(c.categories ?? []); setLocations(l.locations ?? []); }); }, []);
